...
class GoodReadsS3Module:
    ...
    def __init__(self):
        self._s3 = boto3.resource(service_name = 's3', region_name = 'us-east-1', aws_access_key_id=config.get('AWS', 'Key'), aws_secret_access_key=config.get('AWS', 'SECRET'))
        ...
        def s3_move_data(self, source_bucket = None, target_bucket = None):
            ...
            :param target_bucket: name of target buget
            """
            ...
            for key in self.get_files(source_bucket):
                if key in config.get('FILES', 'NAME').split(","):
                    ...
                    # Cleanup source bucket,
                    # Cleaning bucket part is commented to avoid uploading files to s3
                    # again and again when testing heavy loads on ETL
